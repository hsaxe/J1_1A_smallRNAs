...
```{r}
wide = long2 %>% 
  pivot_wider(names_from = chromosome2,
              values_from = c(percent,
                              sum_counts)) %>% 
  arrange(desc(percent_pDE00.0201_resequenced))

# Create the Excel file
wb <- createWorkbook()

addWorksheet(wb, "Counts_by_Chromosome")

# Write the data frame to the Excel file
writeData(wb, 
          "Counts_by_Chromosome", 
          wide)
...
saveWorkbook(wb, 
             file = "DGEresults/Fig3_1A_percentages_table.xlsx", 
             overwrite = TRUE)
```
